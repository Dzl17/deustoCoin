...
def home():
    ...
    s = Session()
    query = s.query(User)
    dictupdate = {User.blockHash: "0x59b767B36A3830f9467038680f737E38cEEaDa02"}
    query.filter(User.email == "javifuenn@gmail.com").update(dictupdate, synchronize_session=False)
    s.delete(query)
    query2 = s.query(User)
    query2 = query2.filter(User.email == "cristianoesjudio@gmail.com").first()
    s.delete(query2)
